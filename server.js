...
// ─── Azure proxy — optimised for speed ────────────────────────────────────────
// Keep-alive agent reuses TCP connections to Azure, drastically cuts TTFT
...
max_tokens: payload.max_tokens || 2048,   // Reduced from 4096 — less wait, faster first token
temperature: payload.temperature ?? 0.3,   // Lower temp = faster + more deterministic
...
agent: azureAgent,  // Reuse TCP connections
};
res.writeHead(200, {
...
// Forward all chunks immediately — do not buffer
azRes.on("data", chunk => {
res.write(chunk);
// Flush immediately so client sees tokens right away
if (res.flush) res.flush();
});
azRes.on("end", () => res.end());
azRes.on("error", e => { if (!res.writableEnded) res.end(); });
});
azReq.on("error", e => {
